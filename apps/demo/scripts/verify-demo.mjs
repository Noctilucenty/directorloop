...
const landing=fs.readFileSync(new URL("../src/Landing.tsx",import.meta.url),"utf8");assert.ok(landing.includes("Find what loses the viewer."));assert.ok(landing.includes("createLocalPreview(file)"));assert.ok(landing.includes("return preview.release"));assert.ok(landing.includes('LiveAnalysis'));
