...
import {findingForMoment,sectionRange,riskLabel,type ReviewCheck,type AttentionAssessment} from './review-state';
import {decidePoll,repairProgress,workflowEventMessage,type ReviewRepair} from './poll-state';
...
type Window={start_ms:number;end_ms:number;status:string;validation_issues:string[];weave_url?:string;attention_assessment?:AttentionAssessment|null;judgment:null|{understanding:string;attention_risk:string;suggestion:string;uncertainties:string[];observations:Observation[];review_checks?:ReviewCheck[]}};
type Report={status:string;model_calls:number;weave_url?:string;windows:Window[];error?:string|null;duration_ms:number;scorecard?:Scorecard|null;review_repair?:ReviewRepair|null};
...
export default function LiveAnalysis({source,mode,onBusy}:{source:Source;mode:'single'|'ab';onBusy:(value:boolean)=>void}){
 ...
 async function api(path:string,init:RequestInit={}){
  ...
  try{const response=await fetch(ENDPOINT+path,{...init,cache:'no-store',signal:controller.signal,headers:{'X-Demo-Session':session.current,...init.headers}});
 ...
 return <><div className="input-actions"><span className="muted">{mode==='ab'?'Preview two versions here. The recorded experiment below includes agent edits and comparison.':'Whole-video review · up to 8 steps'}</span><span className="input-buttons">{mode==='single'&&<button className="btn btn-primary btn-hero" disabled={!source||mode!=='single'||busy||pollPaused} onClick={()=>void start()}>{busy?'Analyzing…':pollPaused?'Updates paused':'Analyze video'}</button>}<a className={mode==='ab'?'btn btn-primary':'btn btn-quiet'} href="#experiment">{mode==='ab'?'See a tested edit':'Explore a demo'}</a></span></div><button type="button" className="text-link replay-review" disabled={busy||pollPaused} onClick={replaySaved}>View an example review</button>{connected===false&&!busy&&<div className="analysis-availability"><div role="status"><p>{availability.message}</p><p>{availability.detail}</p></div><button type="button" className="btn btn-small" disabled={checkingAvailability} onClick={()=>void checkAvailability()}>{checkingAvailability?'Checking availability…':'Check availability'}</button></div>}
 ...
 {events.length>0&&<details className="live-progress" open={busy}><summary>Actual workflow progress · {events.length} events</summary><ol>{events.map(e=><li key={e.seq}><span className="mono">{String(e.seq).padStart(2,'0')}</span><span>{workflowEventMessage(e)}</span></li>)}</ol></details>}
 {ids&&!busy&&!pollPaused&&<button type="button" className="text-link" onClick={checkProgress}>Refresh saved result</button>}
 {!busy&&result?.review_repair&&repairProgress(result.review_repair)&&<p className="live-note">{repairProgress(result.review_repair)}</p>}
 {result&&<><ScoreSummary scorecard={result.scorecard} reportStatus={busy?'running':result.status} windows={result.windows}/><RiskChart windows={result.windows} duration={result.duration_ms} timeline={result.scorecard?.timeline} improvements={result.scorecard?.improvements}/><details className="complete-evidence"><summary>Evidence details and Weave trace</summary>{isReplay&&<p className="live-note">The example uses short editorial summaries of saved model suggestions. The original wording is preserved below.</p>}<div className="live-result-meta"><span>{result.model_calls} model calls</span><span>{result.duration_ms>0?(result.duration_ms/1000).toFixed(1)+'s video':'Preparing media'}</span>{trace(result.weave_url)&&<a href={trace(result.weave_url)} target="_blank" rel="noreferrer">Open live Weave trace ↗</a>}</div><div className="live-evidence-list">{result.windows.map((w,index)=><details className="live-evidence-row" key={w.end_ms}>
